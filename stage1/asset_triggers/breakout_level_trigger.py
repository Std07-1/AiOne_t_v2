...
def breakout_level_trigger(
    ...
    if len(df) < window + 1:
        logger.debug(
            "[%s] Недостатньо даних: %d < window+1=%d",
    ...
    if near_threshold > 0:
        triggers["near_high"] = (recent_high - current_close) / recent_high < near_threshold
        triggers["near_low"]  = (current_close - recent_low)  / recent_low  < near_threshold
    ...
    logger.debug(
        ...
        symbol,
        triggers["breakout_up"], triggers["breakout_down"],
        triggers["near_high"], triggers["near_low"],
        triggers["near_daily_support"], triggers["near_daily_resistance"],
