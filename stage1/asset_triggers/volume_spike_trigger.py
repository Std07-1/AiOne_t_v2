...
def volume_spike_trigger(df: pd.DataFrame, z_thresh: float = 2.0, atr_window: int = 14, symbol: str = "") -> bool:
    ...
    if len(df) < atr_window:
        logger.debug(f"[{symbol}] [VolumeSpike] Недостатньо даних ({len(df)}) для ATR={atr_window}")
        return False
