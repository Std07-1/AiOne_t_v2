...
def rsi_divergence_trigger(df: pd.DataFrame, rsi_period: int = 14, symbol: str = "") -> dict:
    ...
    if len(close) < rsi_period + 3:
        logger.debug(f"[{symbol}] [RSIDivergence] Недостатньо даних ({len(close)}) для rsi_period={rsi_period}")
    ...
    if len(peak_indices) >= 2:
        ...
        if prices[last_peak] > prices[prev_peak] and rsi_series.iloc[last_peak] < rsi_series.iloc[prev_peak]:
            bearish_div = True
    if len(trough_indices) >= 2:
        last_trough = trough_indices[-1]
        prev_trough = trough_indices[-2]
        if prices[last_trough] < prices[prev_trough] and rsi_series.iloc[last_trough] > rsi_series.iloc[prev_trough]:
            bullish_div = True
    logger.debug(
