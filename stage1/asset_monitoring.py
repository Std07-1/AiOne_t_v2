...
from stage1.utils import standardize_format
...
class AssetMonitorStage1:
    ...
    async def ensure_symbol_cfg(self, symbol: str) -> Thresholds:
        ...
        if symbol not in self._symbol_cfg:
            ...
            if thr is None:
                logger.warning(f"[{symbol}] Не знайдено порогів у Redis, використовую стандартні")
                thr = Thresholds()
    ...
    async def update_statistics(
        ...
        rsi_s = compute_rsi(df["close"], symbol=symbol)   # Для статистики (векторний, не обов’язково на кожен бар)
        ...
        over = getattr(self, "rsi_overbought", None) or min(avg_rsi * getattr(self, "dynamic_rsi_multiplier", 1.25), 90)
        under = getattr(self, "rsi_oversold", None) or max(avg_rsi / getattr(self, "dynamic_rsi_multiplier", 1.25), 10)
    ...
    async def check_anomalies(
        ...
        if atr_pct < thr.low_gate:
            logger.debug(f"[{symbol}] ATR={atr_pct:.4f} < поріг low_gate — ринок спокійний.")
            return {
                "symbol": symbol,
                "current_price": price,
                "signal": "NORMAL",
        ...
        low_atr_flag = False
        if atr_pct < thr.low_gate:
            logger.debug(f"[{symbol}] ATR={atr_pct:.4f} < поріг low_gate — ринок тихий, але продовжуємо аналіз.")
            low_atr_flag = True
        ...
        vwap_trig = vwap_deviation_trigger(self.vwap_manager, symbol, price, threshold=0.005)
        if vwap_trig["trigger"]:
            _add("vwap_deviation", f"⚖️ Відхилення від VWAP на {vwap_trig['deviation']*100:.2f}%")
    ...
    #        _add("oi_spike", "🆙 Сплеск відкритого інтересу (OI)")
        ...
        signal = (
            "ALERT"
            if len(reasons) >= self.min_reasons_for_alert
            else "NORMAL"
        )

        logger.debug(
            f"[{symbol}] SIGNAL={signal}, тригери={reasons}, ціна={price:.4f}"
        )
...
async def screening_producer(
    ...
    lookback: int = 20,
    interval_sec: int = 60,
    min_ready_pct: float = 0.2,   # Запускаємо коли ≥20% активів мають ≥lookback барів
):
    logger = logging.getLogger("screening_producer")
    logger.info(
        "🔄 Скринінг активів запущено. (initial assets=%d, tf=%s)",
        len(assets), timeframe
    )
    
    # пояснюємо поріг тільки один раз на старті
    assets_current = assets[:]  # копія для динаміки
    min_ready = max(1, int(len(assets_current) * min_ready_pct))
    ready_symbols = set()
    progress_logged = set()

    while True:
        # --- 3.3 Динамічне оновлення списку активів зі Redis ---
        # але ігноруємо пустий результат (ключ відсутній)
        raw = await cache_handler.get_fast_symbols()
        if raw:
            new_assets = [s.lower() for s in raw]
        else:
            # Redis повернув None/порожній список — пропускаємо оновлення
            new_assets = assets_current

        if set(new_assets) != set(assets_current):
            added = set(new_assets) - set(assets_current)
            removed = set(assets_current) - set(new_assets)
            logger.info(
                "Оновлення assets: +%s, -%s → new total=%d",
                sorted(added), sorted(removed), len(new_assets)
            )
            assets_current = new_assets
            # оновлюємо min_ready якщо змінився розмір
            min_ready = max(1, int(len(assets_current) * min_ready_pct))
        ...
        # Збираємо батч для всіх активів + BTCUSDT (еталон)
        for sym in set(assets_current + [reference_symbol]):
            bars = buffer.get(sym, timeframe, lookback)
            batch[sym] = pd.DataFrame(bars) if bars else pd.DataFrame()
            if sym in assets:
                bar_count = len(bars)
                bars_status.append((sym, bar_count))
                if bar_count >= lookback:
                    ready_count += 1
                    ready_symbols.add(sym)

        # ---- Агреговане логування ----
        logger.debug(
            "[Screening] Прогрес: %d із %d символів мають ≥%d барів",
            ready_count, len(assets_current), lookback
        )

        # Якщо замало даних — не оновлюємо UI, не пушимо batch
        if ready_count < min_ready:
            await asyncio.sleep(10)
            continue

        # ---- Тепер формуємо тільки по тим, хто має ≥lookback барів ----
        for sym, bar_count in bars_status:
            if bar_count < lookback:
                continue
            # 1) Повна історія для seed‑індикаторів (хоча б period+1 барів)
            bars_full = buffer.get(sym, timeframe, lookback)
            df_full = pd.DataFrame(bars_full)
            # 2) «Малий» df тільки для тригерів/UI
            bars_small = buffer.get(sym, timeframe, lookback)
            df_small = pd.DataFrame(bars_small)
    
            # 3) Ініціалізуємо RSI/ATR/VWAP на повному df
            stats = await monitor.update_statistics(sym, df_full)
            # 4) Перевіряємо тригери тільки на df_small
            res = await monitor.check_anomalies(sym, df_small, stats=stats)
            results.append(res)

        # --- Відправка результатів у чергу для stage2/consumer ---
        if not results:
            logger.info("[Screening] Немає жодного активу із достатньою історією для аналізу.")
        else:
            if queue.full():
                _ = queue.get_nowait()
