...
class RAMBuffer:
    ...
    def add(self, symbol: str, timeframe: str, bar: dict):
        ...
        if tf_deque and tf_deque[-1].get("timestamp") == bar.get("timestamp"):
            ...
            logger.debug(
                ...
                symbol, timeframe, bar.get("timestamp")
            )
        else:
            ...
            logger.debug(
                ...
                symbol, timeframe, prev_count, len(tf_deque), bar.get("timestamp")
            )
    ...
    def get(self, symbol: str, timeframe: str, count: int) -> list:
        ...
        logger.debug(
            ...
            len(bars), count, symbol, timeframe, len(self.data[symbol][timeframe])
        )
        return bars
    ...
    def stats(self) -> dict:
        ...
        stat = {
            ...
            "by_symbol": {sym: {tf: len(self.data[sym][tf]) for tf in self.data[sym]} for sym in self.data}
        }
        logger.info(
            "[RAMBuffer] Статистика: %d символів, %s", stat["symbols"], {
    ...
    def symbols_with_min_bars(self, timeframe: str, min_bars: int) -> list:
        ...
        """
        result = [
            sym for sym in self.data
            if len(self.data[sym][timeframe]) >= min_bars
        ]
        logger.info(
            "[RAMBuffer] %d символів мають ≥%d бар(ів) у %s",
            len(result), min_bars, timeframe
        )
        return result
