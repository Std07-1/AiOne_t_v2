...
def safe_float(value: Any, default: float = 0.0, name: Optional[str] = None) -> float:
    """
    Безпечне приведення до float із логуванням при невдачі.

    Args:
        value: Вхідне значення.
        default: Значення за замовчуванням.
        name: Ім'я поля для логів.

    Returns:
        float: Результат конвертації або default.
    """
    try:
        if value is None:
        ...
        logger.warning(
            f"safe_float: не вдалося конвертувати{field}, value={value!r}, default={default}"
        )
        return default
